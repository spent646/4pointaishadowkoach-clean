    ...
    def send_audio(self, stream_label: str, audio_data: bytes):
        ...
        with self._lock:
            ...
            if self.deepgram_available and self.api_key:
                ...
                # This is just for testing the pipeline without Deepgram
                import random
                if not hasattr(self, '_placeholder_counter'):
                    self._placeholder_counter = {}
                
                if stream_label not in self._placeholder_counter:
                    self._placeholder_counter[stream_label] = 0
                
                self._placeholder_counter[stream_label] += 1
                
                # Generate a test transcript every ~50 audio chunks (~1 second)
                if self._placeholder_counter[stream_label] % 50 == 0:
                    test_phrases = [
                        "This is a test transcript from stream",
                        "Placeholder transcription is working",
                        "Audio bytes are being received",
                        "Testing the coach integration",
                        "The microphone is capturing audio",
                        "System audio is being monitored"
                    ]
                    test_text = random.choice(test_phrases) + f" {stream_label}"
                    callback = self.streams[stream_label]["callback"]
                    callback(test_text, True)  # True = final result
